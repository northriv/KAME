...
Scripting nodes: `Tuning` (start/monitor tuning), `Target` (target frequency), `ReflectionTargeted`/`ReflectionRequired` (tuning goals), `UseSTM1`/`UseSTM2` (which stepper-motor axes to drive), `FitFunc`, `AbortTuning`; status readbacks `Succeeded`, `FitStatus`, and the fitted circuit values `L1`, `R1`, `R2`, `C1`, `C2`. Preset-angle helpers: `AddPresetAngles`, `TrustPresetAnglesInPercent`, `DescPresetAngles`.
