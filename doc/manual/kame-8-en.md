...
**Safety — raising temperature in a cryogenic experiment.** In a low-temperature setup, **raising `TargetTemp` above ~295 K (room temperature) requires explicit user confirmation** — warming a cold cryostat can boil off cryogen, stress seals/samples, or exceed equipment limits. Also note the loop **can overshoot by 10% or more**: a 300 K target may transiently swing to ~330 K, so never pick a target on the assumption that the setpoint is the peak — leave margin below any damage threshold. An AI may approach a higher temperature **only if it actively monitors the real temperature** (read the Scalar Entry / poll `Stabilized`) while waiting and is prepared to hold or back off — e.g. step `TargetTemp` up gradually and watch the reading converge at each step — rather than commanding a high target and walking away.
...
To move from a script: make sure `Active` is true (excitation on), then **set `Target` — writing it starts the move** to that absolute position. Poll `Ready` (true once settled; `Slipping` flags a step-out fault) before reading the position from the Scalar Entry. `StopMotor` halts immediately (and resets `Target` to the current position); `GoHomeMotor` returns to the origin.

**Safety — read before moving a motor from a script or AI.** The reported `Position` is not always a true measured position. When `HasEncoder` is false the position is an **open-loop estimate** dead-reckoned from the step count, and some stages (e.g. the SigmaOptics PAMC-104 piezo positioner) **do not report a meaningful position at all**. A motor may also have a limited safe range, and moving it can be **irreversible** — there may be no way to recover the original, known-good position (a probe/coil can collide, a sample can be lost, an LC capacitor can run past its usable band). Therefore an automated agent must **not** jump a motor to an arbitrary `Target`. Instead: (1) confirm carefully with the user — the intended axis, direction, magnitude, and the safe range — before any motion; (2) move in **small increments**, watching another driver’s response (the tuning reflection, a signal amplitude, an optical reading, …) as feedback after each step, and stop the moment it diverges from the expectation; (3) never assume the move can be undone. For LC tuning specifically, prefer the Auto LC Tuner driver (which closes the loop on a network analyzer) over commanding the motors directly.

**`GoHomeMotor` is especially dangerous.** Homing drives the motor until a home sensor is reached — but if no home sensor is installed, **it never stops and the motor runs forever** (until something is damaged or the user intervenes). Crucially, **neither KAME nor the hardware can tell whether a home sensor exists** — only the user knows. So `GoHomeMotor` must be treated as unsafe by default: an automated agent must **never** issue `GoHomeMotor` without first explicitly asking the user to confirm that a home sensor is present on that specific axis.
...
Scripting nodes: source drivers `DMM`, `DCSource` (select which instruments to use), `DMMChannel`, `Control` (start/stop the reversing cycle); `Resistance` is the readback (also sent to the Scalar Entry).
...
Scripting nodes: source drivers `Pulser`, `DSO`; analysis window `FromTrig` (“Position from Trigger”), `Width`, `NumEcho` (“Echo Count”), `EchoPeriod`; background `BGPos`, `BGWidth`, `UsePNR` (periodic noise removal); FFT `FFTPos` (“FFT Origin”), `FFTLen`, `WindowFunc`, `WindowLength`; accumulation `ExAvgIncr` (“Incremental”), `ExtraAvg`, `AvgClear` (“Erase”), `PhaseAdv`, `DIFFreq`, `VoltLimit`; `SpectrumShow` opens the spectrum panel.
...
Scripting nodes: source drivers `PulseAnalyzer`, `MagnetPS` (the field source — may be a magnet PS, a DMM, or a QD PPMS); `CenterFreq`, `Resolution` (“Resolution (Approx.)”), `FieldMin`, `FieldMax`, `FieldFactor`, `ResidualField`; plus the FSS/FFT shared nodes `BandWidth`, `AutoPhase`, `Phase`, `WindowFunc`, `WindowWidth`, `SpectrumSolver`, `Clear`. Field control itself must be driven separately.
...
Scripting nodes: source drivers `NetworkAnalyzer` (the VNA it watches), `STM1`/`STM2` (the stepper-motor drivers for the two tuning capacitors), and optional `RelayDriver` (its AUX bit toggles an RF relay around a tune); `Target` (target frequency), `ReflectionTargeted`/`ReflectionRequired` (tuning goals), `UseSTM1`/`UseSTM2` (which axes to drive), `FitFunc`; status readbacks `Tuning` (true while a tune is in progress), `Succeeded`, `FitStatus`, and the fitted circuit values `L1`, `R1`, `R2`, `C1`, `C2`; `AbortTuning` (touchable) stops a tune. Preset-angle helpers: `AddPresetAngles`, `TrustPresetAnglesInPercent`, `DescPresetAngles`.

To tune from a script: select `NetworkAnalyzer` and the `STM1`/`STM2` motor drivers, enable `UseSTM1`/`UseSTM2`, then **set `Target` — writing the target frequency starts the tune** (it activates the motors and sets `Tuning` true). Poll `Tuning` until false, then read `Succeeded` (and `FitStatus`) for the outcome; call `AbortTuning` to give up. The tuner drives the motors itself, so do not also command `STM1`/`STM2` `Target` while a tune is running.
