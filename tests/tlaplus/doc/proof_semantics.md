...
- `acquire_tag_ref_` 等の primitive 内部の CAS retry は **単一 atomic location 上の単一 CAS retry** パターン。敗者は再 load して再 CAS するだけ、追加作業ゼロ。各 CAS round に hardware が 1 名の勝者を保証するため、構造的 lock-free。
...
- 各 primitive (`acquire_tag_ref_`, `release_tag_ref_`, `compareAndSwap_(NOSWAP=true)`, `swap`, `reset`) について「単一 atomic location 上の単一 CAS retry」パターンであることを informal に確認
