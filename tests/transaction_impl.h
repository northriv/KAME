...
template <class XN>
Node<XN>::Packet::Packet(const shared_ptr<atomic_shared_ptr<Packet> > &branchpoint) :
...
template <class XN>
Node<XN>::Node() : m_packet(new atomic_shared_ptr<Packet>(
...
template <class XN>
void
Node<XN>::initPayload(Payload *payload) {
		...
		if(hint) {
			...
			if(supernodelist &&
				...
				if(superpacket &&
					...
					local_shared_ptr<Packet> &foundpacket((*superpacket)->subpackets()->at(hint->m_index));
...
template <class XN>
inline bool
Node<XN>::trySnapshotSuper(atomic_shared_ptr<Packet> &branchpoint, local_shared_ptr<Packet> &target) {
...
Node<XN>::unbundle(atomic_shared_ptr<Packet> &branchpoint,
