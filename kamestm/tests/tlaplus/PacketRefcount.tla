...
VARIABLES
    ...
    scopeW,      \* [Threads -> Wrappers \cup {Null}]
    viewHeld,    \* [Threads -> BOOLEAN]  does the view still protect it?
    pc,          \* [Threads -> String]
    deadRead     \* BOOLEAN: a thread read a packet through a live path
                 \*          whose rc was 0 (the §13.63 observation)

vars == <<rc, holders, wAlive, wPacket, wrc, link, scopeW, viewHeld, pc, deadRead>>
...
ScopeAcquire(t, n) ==
    ...
    /\ UNCHANGED <<rc, holders, wAlive, wPacket, link, deadRead>>
...
SuperWrapperCopy(t) ==
    ...
    /\ UNCHANGED <<wAlive, wPacket, wrc, link, scopeW, viewHeld>>
...
CommitCAS(t) ==
    ...
    /\ \E p \in Packets : HolderS(t) \in holders[p] /\
       LET old == scopeW[t]
           n   == old[1]
           new == <<n, 1>>
           dies == (IF BUG_LIFETIME THEN wrc[old] - 2 <= 0
                                    ELSE wrc[old] - 1 = 0)
       IN /\ link[n] = old            \* CAS precondition: unchanged
          ...
          /\ wrc'  = [wrc  EXCEPT ![new] = 1,
                       ![old] = IF BUG_LIFETIME THEN @ - 2 ELSE @ - 1]
...
RestoreView(t) ==
    ...
    /\ LET n == scopeW[t][1] IN
       ...
       /\ viewHeld' = [viewHeld EXCEPT ![t] = TRUE]
       /\ wrc'      = [wrc      EXCEPT ![link[n]] = @ + 1]
    /\ pc' = [pc EXCEPT ![t] = "scoped"]     \* may read again
    /\ UNCHANGED <<rc, holders, wAlive, wPacket, link, deadRead>>
...
DropLocals(t) ==
    ...
    /\ UNCHANGED <<wAlive, wPacket, wrc, link, scopeW, viewHeld, pc, deadRead>>
...
Next ==
    \/ \E t \in Threads, n \in Nodes : ScopeAcquire(t, n)
...
TypeOK ==
    /\ \A p \in Packets : rc[p] \in 0..8
    /\ \A w \in Wrappers : wrc[w] \in 0..8
