...
XNMRT1::XNMRT1(const char *name, bool runtime,
  ...
  : XSecondaryDriver(name, runtime, scalarentries, interfaces, thermometers, drivers),
    ...
    m_pulse1(create<XItemNode < XDriverList, XNMRPulseAnalyzer > >("NMRPulseAnalyzer1", false, drivers)),
  ...
  if( emitter != shared_from_this() ) {
      ...
      if(_active) {
        ...
        if(m_ignoredCount <= (int)*ignoreCount()) {
            m_statusPrinter->printMessage(KAME::i18n("Ignored"));   
            return;
        }
        m_avgCount++;
      ...
      switch(_pulser->combModeRecorded()) {
        ...
        case N_COMB_MODE_COMB_ALT:
          ...
          if(_pulse2) {
            ...
            throw XRecordError(KAME::i18n("2 Pulse Analyzers needed."), __FILE__, __LINE__);
         }
