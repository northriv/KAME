...
public:
    ...
    ScopedNegotiateLinkage(LinkagePtr link, Snapshot<XN> &snap, int retry,
        ...
        using SAV = scoped_atomic_view<PacketWrapper>;
        const auto thr = we_hold_priv ? SAV::DEFER_THRESHOLD
                                      : SAV::ADAPTIVE_THRESHOLD;
        m_view = SAV(*m_link, thr, /*weakly=*/!we_hold_priv);
        if(!m_view.acquire_succeeded()) {
            if(we_hold_priv) {
                // STRONG mode never returns acquire_succeeded()==false
                // (it spins until success) — this branch is unreachable.
                m_contention_observed = true;
            } else {
                // Non-privileged weak acquire CAS lost (or tag at capacity).
                // Before escalating to the expensive DISTURBED→retry_pause→
                // negotiate cycle, spin a bounded number of times: tag slots
                // turn over in ~1 CAS round-trip, so a small spin covers the
                // typical congestion window and avoids retire overhead that
                // dominates at high thread counts (N >> LOCAL_REF_CAPACITY).
                for(int i = 0; i < (int)SAV::ADAPTIVE_THRESHOLD; ++i) {
                    pause4spin();
                    m_view = SAV(*m_link, SAV::ADAPTIVE_THRESHOLD,
                                 /*weakly=*/true);
                    if(m_view.acquire_succeeded())
                        break;
                }
