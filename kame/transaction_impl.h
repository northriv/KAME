...
Node<XN>::insert(Transaction<XN> &tr, const shared_ptr<XN> &var, bool online_after_insertion) {
    ...
    // retry loop re-snapshots and tries again.  Without this guard the
    // throw from the 4-arg overload propagates out of iterate_commit
    // (which only catches bad_alloc) and terminates the process.
