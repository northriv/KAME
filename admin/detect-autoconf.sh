...
checkAutoconf()
{
  ...
  elif test -x "`$WHICH autoconf-2.59 2>/dev/null`" ; then
    AUTOCONF="`$WHICH autoconf-2.59`"
  elif test -x "`$WHICH autoconf-2.58 2>/dev/null`" ; then
...
checkAutoheader()
{
  ...
  elif test -x "`$WHICH autoheader-2.58 2>/dev/null`" ; then
    AUTOHEADER="`$WHICH autoheader-2.58`"
    AUTOM4TE="`$WHICH autom4te-2.58`"
...
{
 if test -x "`$WHICH automake-1.8 2>/dev/null`" ; then
    AUTOMAKE="`$WHICH automake-1.8`"
    ACLOCAL="`$WHICH aclocal-1.8`"
  elif test -x "`$WHICH automake-1.6 2>/dev/null`" ; then
