...
exists(../../tests/tests.pri):!win32-*g++ {
    # ---- Monorepo (inside kame), non-MinGW (non-Windows, or Win-MSVC) ----
    ...
    #
    # MinGW Windows (win32-*g++) is excluded: PE/COFF can't export
    # `operator new` from libkamepoolalloc.dll, and the linker binds the
    # test's new/delete to libc++'s defaults regardless of link order — so
    # the pool would never be exercised (no "Reserve swap space"; symptom #2
    # of the MinGW handoff).  MinGW falls through to the inline-compile
    # branch below, which gives this self-contained allocator test its OWN
    # pool `operator new` (exactly as kame.exe does).  MSVC (win32-msvc*) is
    # NOT excluded here — it builds USE_STD_ALLOCATOR (no pool at all), so
    # the symptom-#2 fix is moot there; keep its existing DLL-link path
    # rather than alter an untested build config.
    include(../../tests/tests.pri)
} else {
    # ---- Standalone kamepoolalloc repo, OR MinGW Windows monorepo ----
    # tests.pri (and kamestm) are absent (standalone) or skipped (MinGW Win),
