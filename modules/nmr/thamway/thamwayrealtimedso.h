...
private:
//    enum {ChunkSize = 16*1024,//1MB transfer each. >64kB is fine with libusb. <=4MB for CyUSB3.sys.
          enum {ChunkSize = 64*1024,//1MB transfer each. >64kB is fine with libusb. <=4MB for CyUSB3.sys.
    ...
    std::vector<unique_ptr<XThread>> m_acqThreads;
    XMutex m_acqMutex;
    bool m_swapTraces;
