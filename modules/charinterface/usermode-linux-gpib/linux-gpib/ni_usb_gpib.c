...
{
	struct ni_usb_priv *ni_priv = COMPAT_FROM_TIMER(ni_priv, t, bulk_timer);
					    ...
					    int timeout_msecs)
{
	...
	*actual_data_length = 0;
	...
	if (retval) {
		COMPAT_DEL_TIMER_SYNC(&ni_priv->bulk_timer);
...
static int ni_usb_hs_wait_for_ready(struct ni_usb_priv *ni_priv)
{
	...
	for (i = 0; i < timeout; ++i) {
		...
		if (buffer[++j] != 0x0) { // [6]
			ready = 1;
			// NI-USB-HS+ sends 0xf or 0x19 here; 0x15 seen on some HS firmware
			if (buffer[j] != 0x2 && buffer[j] != 0xe && buffer[j] != 0xf &&
			    buffer[j] != 0x15 && buffer[j] != 0x16 && buffer[j] != 0x19) {
