...
# check_conditional_layout also reads kamestm/ and kamepoolalloc/, whose
# headers are included by every target and so can split a layout the widest).
...
python3 tools/audit/check_conditional_layout.py kame kamestm modules \
    kamepoolalloc || status=1
