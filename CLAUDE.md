...
- **`std::find()` bounds check** — always compare the result of `std::find()` against `end()` before dereferencing. The secondary driver interface's `onItemChanged` was dereferencing without this check.
- **`Payload::local()` requires the GIL** — the comment says "GIL is mandatory" and `gil_scoped_acquire` must actually be present. Deep-copying Python dicts without the GIL corrupts reference counts.
...
- **Base classes with virtual methods need virtual destructors** — `PoolAllocatorBase` and similar base classes that have virtual methods must declare `virtual ~ClassName() = default;` to avoid undefined behavior when deleting through base pointers.
- **Use `||` not `|` for boolean conditions** — bitwise OR does not short-circuit and can cause unintended evaluation. Always use logical OR for boolean expressions.
